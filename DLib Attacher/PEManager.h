...
protected:
	...
	//Расчитывае?виртуальны?размер загруженного образа
	UINT _CalcVirtualRange();
	//Расчитывае?если возможно размер заголовк??буффер?
...
public:
	...
	//Получаем по?дковый номе?секции по имен?
	UINT GetSectorNum(LPSTR name);
	//Получаем по?дковый номе?секции по оффсет?
	UINT GetSectorNum(DWORD voffset);
	//
	INT CheckInSectionOffset(DWORD voffset, UINT sector);
	//Считае?по виртуальному адресу ко?во па?ти свободно?для чтен? из файл?
	UINT CalcInSectionFreeSpace(DWORD voffset, PUINT psector);
	//Выравнивание размер?секции
...
public:
	...
	/* TODO
		- переделать сдви?файло\виртуальны?данных пр?удалении
		+ переделать удаление секций, опреде?ть секцию не по имен??по по?дковом?номеру */
