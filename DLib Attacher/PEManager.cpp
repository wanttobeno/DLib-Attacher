...
BOOL CPEFileManager::ReadHeaderData(LPVOID pbuffer, PUINT psize)
{
	...
	//Пытаем? выделить необходимы?участо?па?ти для заголовк? хо? 0x1000 должно хватат?c первог?раза
...
BOOL CPEFileManager::EditSection(UINT sect_num, PVOID buffer, UINT size, DWORD virt_size)
{
	...
	/*
	+ Если секц? не послед?я, то нужн?убедит? чт?он?не выходи?за свои границ?
	- Если секц? не послед?я, то сдвигаем данные вышест?щи?секций по?новы?размер секции
	*/
	...
	if (vnew_ofst_end > vcurr_ofst_end) {
		for (int i = 0; i < _sect_count; i++) {
			...
			//Проверяем выхо?за границ?виртуально?па?ти
