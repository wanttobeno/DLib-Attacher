...
//Save inject
	...
	if (_pshell->GetFlag(SF_TLS)) {
		if (GetArch() == PE_X86) {
			//Если TLS callback использует? то не ме?ем ег?заголово?
			if (_popt32->DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS].VirtualAddress) {
			...
			//Если TLS callback не использует?, то ме?ем на свой
		...
		if (GetArch() == PE_X86) {
			...
			for (int i = 0; i < size; i++) {
				if (pbuff[i] != 0x00) {
					//TODO выполнит?попытк?расширен? raw данных
			...
			free(pbuff);
			if (!is_clear) {//Пробуе?расширит?raw данные
